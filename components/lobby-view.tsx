...
import { Users, Crown, Copy, Loader2, AlertTriangle, Settings2, Minus, Plus } from "lucide-react";
        ...
        <div className="flex flex-col h-[100dvh] bg-black text-white p-6 relative overflow-hidden">
            ...
            <div className="relative z-10 flex-1 mt-12 overflow-y-auto w-full max-w-2xl mx-auto">
                ...
                <div className="flex items-center justify-between mb-4 px-2">
                    <div className="flex items-center gap-2 text-slate-400 font-mono text-xs uppercase"> {/* Changed text-gray-400 to text-slate-400 */}
                        <Users className="w-4 h-4" />
                        <span>Crew Manifest ({players.length})</span>
                    </div>
                </div>
                        ...
                        {players.map((p) => (
                            <motion.div key={p.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                <PlayerCard
                                    ...
                                    isMe={p.id === me.id}
                                    isDead={p.isDead}
                                    // Kick Action (Host only, on others)
            ...
            {/* Footer Actions */}
            <div className="relative z-20 mt-auto pt-4 border-t border-slate-800 bg-space-black/80 backdrop-blur-sm"> {/* Changed border-white/10 to border-slate-800 and bg-black/80 to bg-space-black/80 */}
                {isHost ? (
                    <GameButton // Changed Button to GameButton
                        variant="primary" // Changed variant and className
                        className="w-full h-16 text-xl"
                        onClick={handleStart}
                        disabled={loading}
                    >
                        {loading ? <Loader2 className="animate-spin mr-2" /> : "Initiate Sequence"}
                    </GameButton>
                ) : (
                    <div className="flex flex-col items-center gap-2 text-center">
                        <Loader2 className="animate-spin text-role-crewmate w-6 h-6" /> {/* Changed text-cyan-500 to text-role-crewmate */}
                        <p className="text-slate-400 text-sm font-mono animate-pulse">Waiting for Host...</p> {/* Changed text-gray-400 to text-slate-400 */}
                    </div>
                )}
